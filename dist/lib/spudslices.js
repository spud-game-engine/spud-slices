...
(function (spudslices) {
    ...
    spudslices.distance = distance;
    ;
    /**
     * Find the rotation (in radians) of (x,y) from (0,1)
     */
    function findRot(x, y) {
        var dist = distance(x, y), pos = [x / dist,
            y / dist];
        if (pos[1] < 0)
            return (2 * Math.PI) - Math.acos(pos[0]); //if it is > pi radians
        return Math.acos(pos[0]);
    }
    spudslices.findRot = findRot;
    ;
    /**
     * Get the location of the point, given that the distance is (x,y) and the
     * rotation to offset it by is `rad`
     */
    function rawRotate(x, y, rad) {
    ...
    spudslices.rawRotate = rawRotate;
    ;
    /**
    ...
    */
    ...
    class Shape {
        constructor() {
            ...
            this.makeDup = function () {
                ...
                for (var iterator = 0, ii; iterator < this.points.length; iterator++) {
                    if (typeof this.points[iterator] == "number")
                        s.points[iterator] = this.points[iterator];
                    else {
                        s.points.push([]);
         ...
         */
        collisionWith(sh) {
            if (typeof spudslices.collisionDetectors[this.category] !== "undefined" &&
                typeof spudslices.collisionDetectors[this.category][sh.category] !==
                    "undefined") {
                return spudslices.collisionDetectors[this.category][sh.category].call(this, sh);
            }
            else if (typeof spudslices.collisionDetectors[sh.category] !== "undefined" &&
                typeof spudslices.collisionDetectors[sh.category][this.category] !==
                    "undefined") {
                var tmp = spudslices.collisionDetectors[sh.category][this.category].call(sh, this);
                if (tmp.length === 0)
                    return [];
                else
                    return [tmp[1], tmp[0]];
                ...
                throw "Could not find shape collision detector for a \"" +
    ...
    spudslices.Circle = Circle;
    spudslices.collisionDetectors["circle"] = {};
    spudslices.collisionDetectors["circle"].circle = function (sh) {
        ...
        for (var thsPt = 0; thsPt < ths.segments.length; thsPt++) {
            ...
            thsRadius = ths.points[seg[0]], thsX = ths.points[seg[1]][0], thsY = ths.points[seg[1]][1];
            //iterate through each segment on sha
            for (var shaPt = 0; shaPt < sha.segments.length; shaPt++) {
                var segS = sha.segments[shaPt], /*sha's current segment */ shaRadius = sha.points[segS[0]], shaX = sha.points[segS[1]][0], shaY = sha.points[segS[1]][1];
                if (((distance(thsX - shaX, thsY - shaY) - Math.abs(shaRadius)) - Math.abs(thsRadius)) <= 0)
    ...
    class Polygon extends Shape {
        ...
        constructor(...args) {
             ...
             */
            this.convertToTriangles = function () {
                ...
                var out = [], center = this.findCenter();
                for (var i = 0; i < this.segments.length; i++) {
                    out[i] = new Polygon(this.points[this.segments[i][0]], this.points[this.segments[i][1]], center);
                }
                return out;
            };
            /**
            ...
            this.joinSegments = function (segA, segB) {
                ...
                for (var i = 0; i < this.segments.length; i++) {
                    if (i === segA)
                        this.segments[i] = [a[0], b[1]];
                    if (this.segments[i][0] > a[1])
                        this.segments[i][0]--;
                    if (this.segments[i][1] > a[1])
                        this.segments[i][1]--;
                }
                ...
                for (i = 0; i < this.faces.length; i++) {
                    for (var ii = 0; ii < this.faces[i].length; ii++) {
                        if (this.faces[i][ii] === segB)
                            rm = ii;
                        else if (this.faces[i][ii] > segB)
                            this.faces[i][ii]--;
                    }
                    if (rm >= 0) {
             ...
             */
            this.splitSegment = function (segNum, sugP /*,
            color?: boolean*/) {
                var npoint = (typeof sugP != "undefined" && typeof sugP.length == "number") ?
                    sugP :
                    new Polygon(this.points[this.segments[segNum][0]], this.points[this.segments[segNum][1]]).findCenter(), pointL = this.segments[segNum][0] + 1;
                ...
                for (var i = 0; i < this.segments.length; i++) { //increase the reference indexes for those that are now off by one because of the point spliced in the middle
                    if (this.segments[i][0] >= pointL)
                        this.segments[i][0]++;
                    if (this.segments[i][1] >= pointL)
                        this.segments[i][1]++;
                }
            ...
            for (var i = 0; i < args.length; i++) {
                ...
                if (i > 0) {
                    ...
                    this.segments.push([args.length - 1, i]);
                this.faces[0].push(i);
            }
        }
    }
    spudslices.Polygon = Polygon;
    spudslices.collisionDetectors["polygon"] = {};
    spudslices.collisionDetectors["polygon"].polygon = function (sh) {
        ...
        for (var thsPt = 0; thsPt < ths.segments.length; thsPt++) {
            ...
            thsRX = ths.points[seg[0]][0], thsLX = ths.points[seg[1]][0], thsRY = ths.points[seg[0]][1], thsLY = ths.points[seg[1]][1];
            ...
            for (var shaPt = 0; shaPt < sha.segments.length; shaPt++) {
                ...
                retVal = [[this.points[seg[0]], this.points[seg[1]]],
                    [sh.points[segS[0]], sh.points[segS[1]]]], shaRX = sha.points[segS[0]][0], shaLX = sha.points[segS[1]][0], shaRY = sha.points[segS[0]][1], shaLY = sha.points[segS[1]][1];
                if (shaRX == thsRX &&
                    shaRY == thsRY &&
                    shaLX == thsLX &&
                ...
                */
                ...
                shaM = (shaRY - shaLY) / (shaRX - shaLX), shaB = shaRY - (shaM * shaRX); //find y intercept of shaSegment
    ...
    spudslices.collisionDetectors["polygon"].circle = function (sh) {
        ...
        for (var thsPt = 0; thsPt < ths.segments.length; thsPt++) {
            ...
            for (var shaPt = 0; shaPt < sha.segments.length; shaPt++) {
                var segS = sha.segments[shaPt], /*sha's current segment */ shaX = sha.points[segS[1]][0], shaY = sha.points[segS[1]][1], radius = sha.points[segS[0]];
                if (shaX != 0 || shaY != 0) {
                ...
                /* Below is a complicated equation for circle-line
                    ...
                    */
                var d_x = thsLX - thsRX, d_y = thsLY - thsRY, D = (thsRX * thsLY) - (thsLX * thsRY), d_r = distance(d_x, d_y);
                if ((Math.pow(radius, 2) * Math.pow(d_r, 2)) <
                    Math.pow(D, 2))
                    break; //line doesn't collide.
                var x_p = (((D * d_y) +
                    (Math.sign(d_y) * d_x * Math.sqrt((Math.pow(radius, 2) * Math.pow(d_r, 2)) -
                        Math.pow(D, 2)))) / (Math.pow(d_r, 2))), x_p2 = (((D * d_y) -
                    (Math.sign(d_y) * d_x * Math.sqrt((Math.pow(radius, 2) * Math.pow(d_r, 2)) -
                        Math.pow(D, 2)))) / (Math.pow(d_r, 2)));
                ...
                var y_p = ((-D * d_x) +
                    (Math.abs(d_y) *
                        Math.sqrt((Math.pow(radius, 2) *
                            Math.pow(d_r, 2)) -
                            Math.pow(D, 2)))) / (Math.pow(d_r, 2)), y_p2 = ((-D * d_x) -
                    (Math.abs(d_y) *
                        Math.sqrt((Math.pow(radius, 2) *
                            Math.pow(d_r, 2)) -
                            Math.pow(D, 2)))) / (Math.pow(d_r, 2));
    ...
    spudslices.RightTriangle = RightTriangle;
    class IsosolesRightTriangle extends RightTriangle {
        constructor(x, y, w) {
            super(x, y, w, w);
        }
    }
    spudslices.IsosolesRightTriangle = IsosolesRightTriangle;
    ;
    class Rectagle extends Polygon {
        constructor(x, y, w, h) {
            super([x, y], [x + w, y], [x + w, y + h], [x, y + h]);
        }
    }
    spudslices.Rectagle = Rectagle;
    ;
     ...
     */
    class EqualDistShape extends Polygon {
        /**
         ...
         */
        constructor(x, y, sideCount, radius) {
            var points = [], amountPer = (Math.PI * 2) / sideCount, thusFar = 0;
    ...
    spudslices.EqualDistShape = EqualDistShape;
    ;
